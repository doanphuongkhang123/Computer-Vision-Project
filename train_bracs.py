    ...
    for step, (feats, labels, attn_mask) in enumerate(progress, 1):
        ...
        with torch.set_grad_enabled(training):
            with torch.autocast(device_type="cuda", enabled=amp_enabled):
                logits = model(feats, attn_mask, epoch=0, stride=1)
...
def main():
    parser = argparse.ArgumentParser(description="Train VTransAdaptive on BRACS WSI feature bags.")
    ...
    if args.checkpoint:
        ...
        weights = {
            ...
            if not k.startswith(("adapter.", "importance_gate."))
        }
